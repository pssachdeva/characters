...
class IntrospectionSftDatasetConfig(BaseModel):
...
class IntrospectionSftModelConfig(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class IntrospectionSftTrainingConfig(BaseModel):
    model_config = ConfigDict(extra="forbid")
    ...
    max_length: int = Field(default=3072, gt=0)
    bf16: bool = True
    fp16: bool = False
...
class IntrospectionSftLoraConfig(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class IntrospectionSftTrackingConfig(BaseModel):
    model_config = ConfigDict(extra="forbid")
    ...
    def effective_run_name(self) -> str:
        return self.run_name or "introspection-sft"


class IntrospectionSftConfig(BaseModel):
    model_config = ConfigDict(extra="forbid")
    ...
    lora: IntrospectionSftLoraConfig = Field(default_factory=IntrospectionSftLoraConfig)
    tracking: IntrospectionSftTrackingConfig = Field(default_factory=IntrospectionSftTrackingConfig)


def load_introspection_sft_config(path: str | Path) -> IntrospectionSftConfig:
    ...
    try:
        return IntrospectionSftConfig.model_validate(resolved)
...
def _resolve_paths(raw: object) -> object:
    ...
    if resolved.get("source_trl_config") is not None:
        resolved["source_trl_config"] = str(Path(resolved["source_trl_config"]).resolve())
