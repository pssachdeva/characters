...
from characters.teacher_generation import run_teacher_generation


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", required=True, help="Path to a teacher-generation YAML config.")
    args = parser.parse_args()

    config = load_response_generation_config(args.config)
    summary = run_teacher_generation(config, HostedGenerationBackend())
    print(
        json.dumps(
            {
